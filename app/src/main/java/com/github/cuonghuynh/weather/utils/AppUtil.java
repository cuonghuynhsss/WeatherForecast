...
public class AppUtil {
    ...
    public static boolean getLanguageSelected() {
        ...
        return sharedPreferences.getBoolean("language_saved", true);
    }

    public static void setLanguageSelected(boolean language) {
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(getApp());
        sharedPreferences.edit().putBoolean("language_saved", language).apply();
    }
    ...
    public static String getWeatherStatus(int weatherCode, boolean isRTL) {
        Log.d("aaaaaaa", "bbb" + getLanguageSelected());
        isRTL = getLanguageSelected();
        if (weatherCode / 100 == 2) {
            if (isRTL) {
                return Constants.WEATHER_STATUS_VIET_NAM[0];
            } else {
                return Constants.WEATHER_STATUS[0];
            }
        } else if (weatherCode / 100 == 3) {
            if (isRTL) {
                return Constants.WEATHER_STATUS_VIET_NAM[1];
            } else {
                return Constants.WEATHER_STATUS[1];
            }
        } else if (weatherCode / 100 == 5) {
            if (isRTL) {
                return Constants.WEATHER_STATUS_VIET_NAM[2];
            } else {
                return Constants.WEATHER_STATUS[2];
            }
        } else if (weatherCode / 100 == 6) {
            if (isRTL) {
                return Constants.WEATHER_STATUS_VIET_NAM[3];
            } else {
                return Constants.WEATHER_STATUS[3];
            }
        } else if (weatherCode / 100 == 7) {
            if (isRTL) {
                return Constants.WEATHER_STATUS_VIET_NAM[4];
            } else {
                return Constants.WEATHER_STATUS[4];
            }
        } else if (weatherCode == 800) {
            if (isRTL) {
                return Constants.WEATHER_STATUS_VIET_NAM[5];
            } else {
                return Constants.WEATHER_STATUS[5];
            }
        } else if (weatherCode == 801) {
            if (isRTL) {
                return Constants.WEATHER_STATUS_VIET_NAM[6];
            } else {
                return Constants.WEATHER_STATUS[6];
            }
        } else if (weatherCode == 803) {
            if (isRTL) {
                return Constants.WEATHER_STATUS_VIET_NAM[7];
            } else {
                return Constants.WEATHER_STATUS[7];
            }
        } else if (weatherCode / 100 == 8) {
            if (isRTL) {
                return Constants.WEATHER_STATUS_VIET_NAM[8];
            } else {
    ...
    public static boolean isRTL(Context context) {
        ...
        boolean aaa = directionality == Character.DIRECTIONALITY_RIGHT_TO_LEFT ||
